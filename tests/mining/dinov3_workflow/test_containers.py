...
def config():
    return {
        "execution": {"backend": "external", "capabilities": {"containers": True, "shared_filesystem": True},
...
def test_component_routing_uses_explicit_images():
    value = config()
...
@pytest.mark.parametrize(
    ...
    [
        ("local", {"containers": True, "shared_filesystem": True}),
        ("external", {"containers": False, "shared_filesystem": True}),
        ("external", {"containers": True, "shared_filesystem": False}),
    ],
)
def test_container_config_requires_capable_external_runner(backend, capabilities):
    value = config()
    value["execution"].update(backend=backend, capabilities=capabilities)
...
def test_symbolic_key_fails_without_reading_skill_bank():
