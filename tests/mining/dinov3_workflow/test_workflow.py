...
def test_previous_round_checkpoint_policy_is_explicit_opt_in(
    ...
    round_one = _fake_training_metadata(first_checkpoint)
    round_two = _fake_training_metadata(
        tmp_path / "run/rounds/round_002/train/checkpoint.pth"
    )
...
def test_native_lock_covers_dinov3_and_inherited_nvdinov2_runtime(
    ...
    assert workflow._locked_action_implementation("score").startswith("sha256:")
    assert workflow._locked_action_implementation("train").startswith("sha256:")
