...
from nvidia_tao_ds.mining.dinov3.workflow.controller import StageFailure  # noqa: E402
...
def _committed_payload(
    ...
    artifact["artifact_id"] = canonical_digest(
        {
            key: artifact[key]
            for key in (
                "artifact_type",
...
def test_balanced_multitask_workflow_publishes_unique_and_training_views(
    ...
    checkpoint = json.loads(Path(state["current_checkpoint"]).read_text())
...
def test_every_round_trains_from_immutable_base_checkpoint(
    ...
    round_one = json.loads(
        (tmp_path / "run/rounds/round_001/train/checkpoint.pth").read_text()
    )
    round_two = json.loads(
        (tmp_path / "run/rounds/round_002/train/checkpoint.pth").read_text()
    )
...
def test_legacy_warm_start_maps_to_previous_round_policy(tmp_path: Path) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    value["training"].pop("checkpoint_policy")
    value["training"]["warm_start"] = True
...
def test_conflicting_checkpoint_policy_and_legacy_flag_fail(tmp_path: Path) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    value["training"]["warm_start"] = True

    with pytest.raises(ValueError, match="warm_start conflicts"):
        WorkflowConfig.from_dict(value)


def test_previous_round_checkpoint_policy_is_explicit_opt_in(
    tmp_path: Path,
) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    ...
    base = str(Path(value["model"]["base_checkpoint"]).absolute())
    first_checkpoint = tmp_path / "run/rounds/round_001/train/checkpoint.pth"
    round_one = json.loads(first_checkpoint.read_text())
    round_two = json.loads(
        (tmp_path / "run/rounds/round_002/train/checkpoint.pth").read_text()
    )
    assert round_one["parent"] == base
    assert round_two["parent"] == str(first_checkpoint)
...
def test_parent_checkpoint_requires_parent_training_manifest(tmp_path: Path) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    value["workflow"]["initialization"] = "parent_checkpoint"
...
def test_parent_checkpoint_is_explicit_in_plan(tmp_path: Path) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    parent = tmp_path / "parent.parquet"
    pd.DataFrame({"sample_id": pd.Series(dtype="str")}).to_parquet(
        parent, index=False
    ...
    assert config.plan()["initialization"] == "warm_start_from_parent_checkpoint"


def test_parent_checkpoint_materializes_cumulative_manifest(tmp_path: Path) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    parent = tmp_path / "parent.parquet"
    pd.DataFrame(
        {
...
def test_sealed_training_continuation_is_adopted_then_evaluated(
    tmp_path: Path, balanced_training: bool,
) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    ...
    checkpoint.write_text("accepted teacher", encoding="utf-8")
...
def test_custom_score_and_evaluation_require_implementation_closures(
    tmp_path: Path,
) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
...
def test_generated_stage_request_matches_published_contract_schema(
    tmp_path: Path, containerized: bool,
) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    value["actions"]["evaluate"]["command"] = []
    if containerized:
        value["execution"].update(
            ...
            container_images={"pytorch": "registry/pyt:reviewed", "data_services": "registry/ds:reviewed"},
        )
        value["execution"].setdefault("capabilities", {})["containers"] = True
    workflow = RefinementWorkflow(WorkflowConfig.from_dict(value))
    ...
    assert contract["required_capabilities"] == (["containers"] if containerized else [])
...
@pytest.mark.parametrize("policy", ["base_checkpoint_each_round", "previous_round_checkpoint"])
@pytest.mark.parametrize("failed_stage", ["train", "evaluate"])
def test_retry_preserves_completed_round_inputs(
    tmp_path: Path, policy: str, failed_stage: str,
) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    value["training"]["checkpoint_policy"] = policy
    workflow = RefinementWorkflow(WorkflowConfig.from_dict(value))
    delegate = workflow.runner
    calls = []
...
def test_local_runner_refuses_unallocated_gpu_request(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("CUDA_VISIBLE_DEVICES", raising=False)
    request = StageRequest(
        client_job_id="local-gpu-unallocated",
        run_id="run",
        round_index=1,
        stage="score",
        command=[sys.executable, "-c", "pass"],
...
def test_adapter_managed_uses_one_wrapper_and_records_delegated_allocation(
    ...
    pd.DataFrame({"sample_id": [f"s{index}" for index in range(16)]}).to_parquet(
...
def test_training_preserves_non_gpu_resource_contract(
    ...
    pd.DataFrame({"sample_id": [f"s{i}" for i in range(8)]}).to_parquet(manifest, index=False)
    observed = []
...
def test_legacy_native_leaf_mode_normalizes_to_adapter_managed(
    tmp_path: Path,
) -> None:
    value = _config(tmp_path, "grit_score").to_dict()
    ...
    resolved = WorkflowConfig.from_dict(value).to_dict()

    assert "backend" not in resolved["actions"]["train"]
    assert resolved["actions"]["train"]["execution_mode"] == "adapter_managed"
