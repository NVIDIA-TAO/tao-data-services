...
def main() -> int:
    ...
    if args.command == "score":
        ...
        checkpoint = output_dir / (
            f"teacher_epoch_{int(contract['requested_data_passes']) - 1:03d}_step_"
            f"{int(contract['total_optimizer_steps']):05d}.pth"
        )
        torch.save({"weight": torch.tensor([float(rows)])}, checkpoint)
        (output_dir / "terminal_teacher.json").write_text(json.dumps({
            "schema_version": "1.0", "model": "teacher", "filename": checkpoint.name,
            "epoch": int(contract["requested_data_passes"]) - 1,
            "global_step": int(contract["total_optimizer_steps"]),
            "bytes": checkpoint.stat().st_size, "sha256": _sha256(checkpoint),
        }), encoding="utf-8")
        (output_dir / "fake_training.json").write_text(
            json.dumps(
                {
                    "parent": args.checkpoint,
                    "passes": args.passes,
                    "training_rows": rows,
                    "nodes": contract["num_nodes"],
        ...
        (output_dir / "train_implementation_audit.json").write_text(
            json.dumps(
                {
                    "schema_version": "1.0",
                    "subtask": "train",
                    "entrypoint_sha256": os.environ.get(
                        "TAO_REFINEMENT_ENTRYPOINT_SHA256"
                    ),
                    "implementation_sha256": os.environ.get(
    ...
    if publish_success and not success_path.exists():
