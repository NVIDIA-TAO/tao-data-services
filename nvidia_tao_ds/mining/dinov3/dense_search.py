...
from .ann_search import exact_rerank_ann_candidates, _validate_parameters
...
def _deterministic_numpy_topk(
    scores: np.ndarray, ids: np.ndarray, *, limit: int
) -> tuple[np.ndarray, np.ndarray]:
    """Order candidates by score descending, then global row ID ascending."""
    keep = min(limit, int(scores.shape[1]))
    selected_scores = np.empty((scores.shape[0], keep), dtype=np.float32)
    selected_ids = np.empty((scores.shape[0], keep), dtype=np.int64)
    for query_index in range(scores.shape[0]):
        order = np.lexsort((ids[query_index], -scores[query_index]))[:keep]
        selected_scores[query_index] = scores[query_index, order]
        selected_ids[query_index] = ids[query_index, order]
...
def _numpy_topk(
    ...
    ids = np.broadcast_to(
        np.arange(
            row_offset, row_offset + scores.shape[1], dtype=np.int64
        ),
        scores.shape,
    )
    return _deterministic_numpy_topk(scores, ids, limit=limit)
...
def _deterministic_torch_topk(
    scores: Any, ids: Any, *, limit: int
) -> tuple[Any, Any]:
    """Torch equivalent of the stable score/row-ID ordering contract."""
    keep = min(limit, int(scores.shape[1]))
    id_order = ids.argsort(dim=1, stable=True)
    scores_by_id = scores.gather(1, id_order)
    ids_by_id = ids.gather(1, id_order)
    score_order = scores_by_id.argsort(dim=1, descending=True, stable=True)
    indexes = score_order[:, :keep]
    return scores_by_id.gather(1, indexes), ids_by_id.gather(1, indexes)
...
def exact_dense_candidates(
    *,
    ...
    """
    if candidate_limit <= 0 or chunk_rows <= 0 or checkpoint_chunks <= 0 or query_block_rows <= 0:
    ...
    contract = {
        "schema_version": "1.0",
        "progress_format": "float32_exact_topk_v1",
    ...
    if device == "cpu":
        for start in range(start_row, row_count, chunk_rows):
            ...
            for query_start in range(0, len(queries), query_block_rows):
                query_slice = slice(query_start, query_start + query_block_rows)
                scores = query_vectors[query_slice] @ chunk.T
                local_excluded = _excluded_in_chunk(excluded, start, stop)
                if len(local_excluded):
                    scores[:, local_excluded] = -np.inf
                counts[query_slice] += np.sum(
                    cosine_at_or_above(scores, hard_min_similarity), axis=1
                )
                local_scores, local_ids = _numpy_topk(
                    scores, limit=candidate_limit, row_offset=start
                )
                best_scores[query_slice], best_ids[query_slice] = _merge_numpy_topk(
                    best_scores[query_slice],
                    best_ids[query_slice],
                    local_scores,
                    local_ids,
        ...
        for start in range(start_row, row_count, chunk_rows):
            ...
            chunk = torch.from_numpy(
                np.asarray(vectors[start:stop], dtype=np.float32).copy()
            ).to(device)
            for query_start in range(0, len(queries), query_block_rows):
                query_slice = slice(query_start, query_start + query_block_rows)
                scores = query_tensor[query_slice] @ chunk.T
                local_excluded = _excluded_in_chunk(excluded, start, stop)
                if len(local_excluded):
                    indexes = torch.from_numpy(local_excluded).to(device)
                    scores[:, indexes] = float("-inf")
                count_tensor[query_slice] += cosine_at_or_above(
                    scores, hard_min_similarity
                ).sum(dim=1)
                local_ids = torch.arange(
                    start, stop, dtype=torch.int64, device=device
                ).expand(scores.shape[0], -1)
                local_scores, local_ids = _deterministic_torch_topk(
                    scores, local_ids, limit=candidate_limit
                )
                combined_scores = torch.cat((best_score_tensor[query_slice], local_scores), dim=1)
                combined_ids = torch.cat((best_id_tensor[query_slice], local_ids), dim=1)
                best_score_tensor[query_slice], best_id_tensor[query_slice] = _deterministic_torch_topk(
                    combined_scores, combined_ids, limit=candidate_limit
                )
            chunks_this_attempt += 1
            if chunks_this_attempt % checkpoint_chunks == 0 or stop == row_count:
    ...
    best_scores, best_ids = _deterministic_numpy_topk(
        best_scores, best_ids, limit=candidate_limit
    )
...
def exact_dense_search(
    *,
    ...
    _validate_parameters(top_k=top_k, min_similarity=min_similarity,
                         hard_min_similarity=hard_minimum, similarity_step=similarity_step,
