...
def _vectors_from_array(array: pa.Array, dimension: int) -> np.ndarray:
    ...
    if pa.types.is_fixed_size_list(array.type):
        if int(array.type.list_size) != dimension:
            raise ValueError(
                f"Embedding dimension changed from {dimension} to {array.type.list_size}"
            )
        values = array.values.to_numpy(zero_copy_only=False)
        result = np.asarray(values, dtype=np.float32).reshape(len(array), dimension)
    else:
        result = np.asarray(array.to_pylist(), dtype=np.float32)
    ...
    if np.any(norms == 0):
        raise ValueError("Embedding column contains a zero vector")
    return np.ascontiguousarray(result / norms, dtype="<f4")
...
def _validate_source_shard(source_path: Path, shard: dict[str, Any]) -> None:
    ...
    if expected_identity is not None and current_identity != expected_identity:
        raise RuntimeError(f"Source shard POSIX identity changed: {source_path}")
...
def finalize_dense_store(*, plan_path: str | Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "vector_posix_identity": file_posix_identity(vector_path),
    ...
    artifact = ArtifactManifest(
        ...
        inputs=[file_identity(plan["source_store_manifest"], role="embedding_store")],
        payload=payload,
    )
