...
from .contracts import file_posix_identity
from .dense_store import load_dense_store
from .search import SEARCH_RESULT_COLUMNS, _matrix
    ...
    *,
    ...
    for query_index, raw_ids in enumerate(candidate_row_ids):
        ...
        if len(ids):
            ...
            above = np.flatnonzero(scores >= hard_min_similarity)
            if len(above) > candidate_limit:
                keep = above[
                    np.argpartition(scores[above], -candidate_limit)[-candidate_limit:]
                ]
            else:
                keep = above
...
def _hydrate_locators(frame: pd.DataFrame, dense: dict[str, Any]) -> pd.DataFrame:
    ...
    for output_index in range(len(result)):
        ...
        result.at[output_index, "sample_id"] = str(metadata.pop(dense["id_column"]))
        for name, value in metadata.items():
            if name not in SEARCH_RESULT_COLUMNS or name == "source_part":
