...
from .ann_index import load_ann_index, require_experimental_ann
...
from .search import read_query_table, source_metadata_output_names
...
def retrieve_ann_candidates(
    *,
    ...
    _validated_index: tuple[dict, dict] | None = None,
    ...
    ann, ann_artifact = _validated_index or load_ann_index(ann_index_manifest)
    queries = read_query_table(query_path, ann)
    query_vectors = vector_matrix(queries["embedding"], label="Query embedding")
...
def ann_exact_rerank_search(
    *,
    ...
    if n_probes <= 0 or ann_candidate_count <= 0:
        raise ValueError("n_probes and ann_candidate_count must be positive")
    if ann_candidate_count < top_k:
        raise ValueError("ann_candidate_count must be at least top_k")
    if ann_candidate_count < top_k * candidate_multiplier:
        warnings.warn("ANN candidate depth is below top_k * candidate_multiplier; diversity filtering may underfill",
