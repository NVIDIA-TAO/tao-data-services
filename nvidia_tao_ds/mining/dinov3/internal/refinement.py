...
"""Internal DINOv3 workflow actions, not a registered TAO public entrypoint."""
...
from ..ann_audit import audit_ann_recall
...
def _read_excluded_row_ids(
    ...
    *,
    ...
    if value.suffix == ".parquet":
        ...
        required = {"source_row_id", "source_inventory_digest"}
        ...
        lineage_columns = {
            name
            for name in ("dense_store_artifact_id", "source_store_artifact_id")
            if name in parquet.schema_arrow.names
        }
        if not lineage_columns:
            raise ValueError(
                "Nonempty indexed-search exclusions lack corpus lineage: "
                "dense_store_artifact_id or source_store_artifact_id is required"
            )
        frame = pd.read_parquet(
            value, columns=sorted(required | lineage_columns), pre_buffer=False
        )
        if (
            "dense_store_artifact_id" in frame and
            not set(frame["dense_store_artifact_id"].dropna().astype(str)).issubset(
                {dense_store_artifact_id})
        ):
            raise ValueError("Indexed-search exclusions belong to another dense store")
        if (
            "source_store_artifact_id" in frame and
            not set(frame["source_store_artifact_id"].dropna().astype(str)).issubset(
                {source_store_artifact_id})
        ):
            raise ValueError("Indexed-search exclusions belong to another source store")
        ...
        if frame[sorted(lineage_columns)].isnull().all(axis=1).any():
            raise ValueError("Every indexed exclusion row requires source or dense store lineage")
        if frame["source_row_id"].isnull().any():
            raise ValueError("Indexed-search exclusion rows must have non-null source_row_id")
        return set(frame["source_row_id"].astype(int))
    payload = json.loads(value.read_text(encoding="utf-8"))
    if isinstance(payload, dict):
        row_ids = payload.get("source_row_ids", [])
        if row_ids:
            has_dense = "dense_store_artifact_id" in payload
            has_source = "source_store_artifact_id" in payload
            if not has_dense and not has_source:
                raise ValueError(
                    "Nonempty indexed-search exclusions lack corpus lineage"
                )
            if (
                has_dense and
                payload["dense_store_artifact_id"] != dense_store_artifact_id
            ):
                raise ValueError(
            ...
            if (
                has_source and
                payload["source_store_artifact_id"] != source_store_artifact_id
            ):
                raise ValueError(
                    "Indexed-search exclusions belong to another source store"
                )
...
def _validated_store_identity(
    ...
    if payload.get("inventory_digest") not in {
    ...
    expected_artifact_id = artifact_content_id(artifact)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    audit = commands.add_parser("audit-ann-recall")
    audit.add_argument("--queries", required=True)
    audit.add_argument("--dense-store-manifest", required=True)
    audit.add_argument("--ann-index-manifest", required=True)
    audit.add_argument("--query-embedding-contract", required=True)
    audit.add_argument("--output-dir", required=True)
    audit.add_argument("--n-probes", required=True, type=int)
    audit.add_argument("--ann-candidates", required=True, type=int)
    audit.add_argument("--top-k", type=int, default=10)
    audit.add_argument("--sample-size", type=int, default=128)
    ...
    ann_search.add_argument("--device", default="cpu")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if hasattr(args, "output_dir"):
        with artifact_publication(args.output_dir):
            return _run_command(args)
    return _run_command(args)


def _commit_neighbor_selection(destination, frame, *, search_proof, extra_summary,
                               extra_payload, inputs, producer):
    """Publish one common neighbor/summary/seal contract for all search backends."""
    adaptive = frame.attrs.get("adaptive_radius", {})
    stats = adaptive.get("query_stats", [])
    top_k = extra_payload["top_k"]
    underfilled = any(int(item["selected_count"]) < top_k for item in stats)
    proven = bool(adaptive.get("underfill_exhaustion_proven", False))
    stop_class = ("radius_exhausted" if proven else "search_budget_exhausted") if underfilled else None
    common = {
        "row_count": len(frame), "search_proof": search_proof,
        "candidate_truncated": bool(adaptive.get("candidate_truncated", False)),
        "underfill_exhaustion_proven": proven, "underfill_stop_class": stop_class,
    }
    ...
    write_json_atomic(summary_path, {
        **extra_summary, **common, "query_count": len(stats),
        "selected_query_count": int(frame["query_id"].nunique()) if len(frame) else 0,
        "adaptive_radius": adaptive,
    })
    return ArtifactManifest(
        artifact_type="neighbor_selection", producer=producer, inputs=inputs,
        payload={**extra_payload, **common, "neighbors": file_identity(output),
                 "summary": file_identity(summary_path)},
    ).commit(destination)


def _run_command(args) -> int:
    """Execute one parsed action while its output transaction is reserved."""
    if args.command == "audit-ann-recall":
        result = audit_ann_recall(
            query_path=args.queries, dense_store_manifest=args.dense_store_manifest,
            ann_index_manifest=args.ann_index_manifest,
            query_embedding_contract=args.query_embedding_contract, output_dir=args.output_dir,
            n_probes=args.n_probes, ann_candidate_count=args.ann_candidates,
            top_k=args.top_k, sample_size=args.sample_size, seed=args.seed,
            minimum_recall=args.minimum_recall, device=args.device,
        )
        print(json.dumps(result, indent=2, sort_keys=True))
        return int(not result["payload"]["passed"])
    if args.command == "select-grit":
        ...
        candidate_artifact_id = artifact_content_id(candidate_artifact)
        ...
        excluded_row_ids = _read_excluded_row_ids(
            args.exclude,
            dense_store_artifact_id=dense_artifact["artifact_id"],
            source_store_artifact_id=dense_manifest["source_store_artifact_id"],
            source_inventory_digest=dense_manifest["source_inventory_digest"],
        )
        ...
        frame["source_store_artifact_id"] = dense_manifest[
            "source_store_artifact_id"
        ]
        ...
        _commit_neighbor_selection(
            destination, frame, search_proof=search_proof, extra_summary=summary,
        ...
        validated_index = load_ann_index(args.ann_index_manifest)
        if args.ann_audit_manifest:
            _, ann_artifact = validated_index
            load_ann_audit(
                args.ann_audit_manifest,
                index_artifact_id=ann_artifact["artifact_id"],
                n_probes=args.n_probes,
        ...
        frame = ann_exact_rerank_search(
            ...
            ann_index_manifest=args.ann_index_manifest,
            top_k=args.top_k,
            _validated_index=validated_index,
            min_similarity=args.min_similarity,
            n_probes=args.n_probes,
            ann_candidate_count=args.ann_candidates,
            hard_min_similarity=args.hard_min_similarity,
        ...
        if store_identity is not None:
            frame["source_store_artifact_id"] = store_identity["artifact_id"]
            frame["source_inventory_digest"] = store_identity[
                "inventory_digest"
            ]
        ...
        write_json_atomic(
            summary_path,
            {
                ...
                **({
                    "source_store_artifact_id": store_identity["artifact_id"],
                    "source_inventory_digest": store_identity["inventory_digest"],
                } if store_identity is not None else {}),
