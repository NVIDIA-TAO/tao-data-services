...
from ..contracts import artifact_content_id
...
def _native_implementation_files(
    ...
    """Return the native leaf closure using PyTorch's canonical relative keys."""
    action_root = module.parents[1]
    ...
    if native_subtask == "grit_score":
        ...
        raise ValueError(f"Unsupported native DINOv3 subtask: {native_subtask}")
    implementations = set(required_files)
    ...
    for implementation in sorted(
        (path.resolve() for path in implementations), key=str
    ):
        if not implementation.is_file():
            raise ValueError(
                "Native DINOv3 implementation closure is incomplete: "
                f"{implementation}"
            )
        relative = implementation.relative_to(package_root).as_posix()
        result[f"native.{relative}"] = implementation
    return result
