...
from .snapshots import (
    ...
    snapshot_cache as _snapshot_cache,
    stable_file_snapshot as _stable_file_snapshot,
)
...
def _workflow_source_digest() -> str:
    """Seal the complete built-in DINOv3 DS implementation for restart safety."""
...
def _native_implementation_files(
    module: Path, native_subtask: str,
) -> dict[str, Path]:
    """Use the model-owned, versioned closure; never duplicate its inventory."""
    try:
        from nvidia_tao_pytorch.ssl.dinov3.utils.refinement_attestation import (  # pylint: disable=import-outside-toplevel
            CLOSURE_VERSION, implementation_files,
        )
    except ImportError as error:
        raise ValueError(
            "DINOv3 DEFT requires a compatible TAO PyTorch runtime with "
            "refinement_attestation closure version 1.0"
        ) from error
    if CLOSURE_VERSION != "1.0":
        raise ValueError(f"Unsupported DINOv3 attestation closure: {CLOSURE_VERSION}")
    files = implementation_files(native_subtask)
    entrypoint = files[f"native.ssl/dinov3/scripts/{native_subtask}.py"]
    if entrypoint.resolve() != module.resolve():
        raise ValueError("Controller and configured DINOv3 runtime paths differ")
    return files
...
def _target_identity_set(
    path: Path, strategy: str, expected_embedding_dim: int | None = None,
) -> set[tuple[str, str]]:
    """Stream and validate the exact target identity population."""
    parquet = pq.ParquetFile(path)
    names = set(parquet.schema_arrow.names)
    required = {"sample_id", "task", "embedding"}
    ...
    for batch in parquet.iter_batches(columns=columns, batch_size=16_384):
        frame = batch.to_pandas()
        if (
            frame[["sample_id", "task"]].isnull().any().any() or
            frame["sample_id"].astype(str).str.strip().eq("").any() or
            frame["task"].astype(str).str.strip().eq("").any()
        ):
            raise ValueError(
                "Target sample/task identities must be non-null and non-empty"
            )
        try:
            embeddings = np.asarray(
                frame["embedding"].tolist(), dtype=np.float32
            )
        except (TypeError, ValueError) as exc:
            raise ValueError(
                "Target embeddings must be a finite numeric matrix"
            ) from exc
        if (
            embeddings.ndim != 2 or embeddings.shape[1] == 0 or
            not np.isfinite(embeddings).all()
        ):
            raise ValueError(
                "Target embeddings must be a non-empty finite numeric matrix"
            )
        if embedding_dim is None:
            embedding_dim = int(embeddings.shape[1])
        elif embeddings.shape[1] != embedding_dim:
            raise ValueError(
                "Target embedding dimensions must be consistent"
            )
        batch_sample_ids = frame["sample_id"].astype(str).tolist()
        if (
            len(batch_sample_ids) != len(set(batch_sample_ids)) or
            sample_ids.intersection(batch_sample_ids)
        ):
            raise ValueError("Target sample_id values must be globally unique")
        sample_ids.update(batch_sample_ids)
        if strategy == "grit_score":
            if frame["role"].isnull().any():
                raise ValueError("Target GRIT role values cannot be null")
            roles = set(frame["role"].astype(str))
            unsupported = roles.difference({"query", "reference"})
            if unsupported:
                raise ValueError(
                    f"Target manifest has unsupported GRIT roles: {sorted(unsupported)}"
                )
    ...
    if (
        expected_embedding_dim is not None and
        embedding_dim != expected_embedding_dim
    ):
        raise ValueError(
            "Target manifest embedding dimension differs from "
            "data.target_embedding_contract.embedding_dim"
        )
    return identities
...
class RefinementWorkflow:
    ...
    def validate(self, *, require_paths: bool = True) -> dict[str, Any]:
        ...
        if require_paths:
            ...
            if "target_embedding_contract" in paths:
                try:
                    target_contract = json.loads(
                        paths["target_embedding_contract"].read_text(
                            encoding="utf-8"
                        )
                    )
                except (OSError, json.JSONDecodeError) as exc:
                    raise ValueError(
                        "data.target_embedding_contract must be valid JSON"
                    ) from exc
                if not isinstance(target_contract, dict):
                    raise ValueError(
                        "data.target_embedding_contract must be a JSON object"
                    )
                try:
                    raw_embedding_dim = target_contract["embedding_dim"]
                except KeyError as exc:
                    raise ValueError(
                        "data.target_embedding_contract requires integer "
                        "embedding_dim"
                    ) from exc
                if (
                    isinstance(raw_embedding_dim, bool) or
                    not isinstance(raw_embedding_dim, int)
                ):
                    raise ValueError(
                        "data.target_embedding_contract requires integer "
                        "embedding_dim"
                    )
                expected_embedding_dim = raw_embedding_dim
                if expected_embedding_dim <= 0:
                    raise ValueError(
                        "data.target_embedding_contract.embedding_dim must be "
                        "positive"
                    )
            _target_identity_set(
    ...
    def _exclusion_manifest(
        ...
        if destination.is_file():
            ...
            if (
                ...
                "source_row_id" in present and bool(set(lineage_columns[1:]) & present)
            ):
                return destination
        frames = []
        for path in inputs:
            ...
            if requires_corpus_lineage and parquet.metadata.num_rows:
                if "source_row_id" not in present or not set(lineage_columns[1:]) & present:
                    raise ValueError(
                        "Nonempty row-ID search exclusions require corpus lineage: "
                        f"{path}: source_row_id and at least one corpus identity"
                    )
    ...
    def _data_lock(self) -> dict[str, Any]:
        ...
        for action_name in ("score", "data", "search", "train", "evaluate"):
            ...
            native_subtask = (
                str(action_command[1])
                if len(action_command) >= 2 and str(action_command[0]) == "dinov3"
                else None
            )
            if native_subtask in {"grit_score", "train"}:
                module_name = (
                    f"nvidia_tao_pytorch.ssl.dinov3.scripts.{native_subtask}"
                )
                module = _module_source(module_name, module_paths)
                if module is None:
                    raise ValueError(
                        f"Cannot resolve installed native DINOv3 action {module_name}"
                    )
                adapter_files[f"{action_name}.module.native"] = {
                    "module": module_name,
                    "path": str(module),
                    "sha256": _sha256(module),
                }
                for name, implementation in _native_implementation_files(
                    module, native_subtask
                ).items():
                    adapter_files[
                        f"{action_name}.implementation.{name}"
                    ] = {
    ...
    def _initialize(self) -> dict[str, Any]:
        ...
        if release_path.exists():
            ...
            if any(existing_release.get(key) != release[key]
                   for key in ("workflow", "workflow_version", "schema_version")):
                raise RuntimeError("Workflow compatibility version changed; fork the run")
            if canonical_digest(existing_release) != canonical_digest(release):
                warnings.warn("Controller provenance changed; recording drift. Leaf implementation locks still apply.",
                              RuntimeWarning, stacklevel=2)
                self.store.append_event(self.store.load(), round_index=0, stage="release",
                                        status="provenance_drift", extra={"observed_release": release})
        else:
    ...
    def _run_stage(
        ...
        *,
        ...
        finalize_outputs: Callable[[], None] | None = None,
        recover_outputs: Callable[[], bool] | None = None,
        ...
        job_id = client_job_id(
            run_scope=str(self.run_dir.resolve()),
            run_id=state["run_id"],
            round_index=round_index,
            stage=stage,
            command=command,
        )
        if recover_outputs is not None and recover_outputs():
            try:
                if finalize_outputs:
                    finalize_outputs()
                missing = {
                ...
                if missing:
                    raise RuntimeError(
                        f"recovered action lacks outputs: {missing}"
                    )
                if validate_outputs:
                    validate_outputs()
            except StageFailure:
            ...
            self.store.complete_stage(
                ...
                job={
                    ...
                    "state": "RECOVERED",
                    "native_state": "FINALIZED_WITHOUT_RELAUNCH",
                },
            )
            return
        action_name = {
            ...
            execution_contract={
                ...
                **({
                ...
                "required_capabilities": (
                    ...
                    ["shared_filesystem"]
                    if self.config.value["execution"].get(
                        "capabilities", {}
                    ).get("shared_filesystem") is True
                    else []
                ) + (
                    ["gpu_faiss"]
        ...
        if result.attempt > 1:
            self.store.append_event(state, round_index=round_index, stage=stage, status="retry",
                                    extra={"job": asdict(result), "attempt": result.attempt})
        if result.client_job_id != job_id:
            raise StageFailure(
                stage,
                "Runner returned a result for a different client_job_id: "
                f"{result.client_job_id!r}",
            )
        if (self.run_dir / "cancel.requested").exists():
    ...
    def _locked_action_implementation(self, action_name: str) -> str:
        ...
        entries = lock.get("adapter_files", {})
        prefix = f"{action_name}.implementation."
        implementations = {
        ...
        return canonical_digest(implementations)
    ...
    def _score(self, state: dict[str, Any], round_index: int, round_dir: Path) -> Path:
        ...
        cached = f"round_{round_index:03d}/score" in state["completed_stages"]
        values = {
        ...
        implementation_sha256 = self._locked_action_implementation("score")
        score_request = {
            "schema_version": "1.0",
            "algorithm_version": (
                GRIT_ALGORITHM_VERSION
                if self.config.strategy == "grit_score"
                else "customer_task_score_v1"
            ),
        ...
        if cached:
            if json.loads(request_path.read_text(encoding="utf-8")) != score_request:
                raise StageFailure("score", "Cached score request changed")
        else:
            _atomic_json(request_path, score_request)
        audit_path = output_dir / "score_audit.json"
        expected_audit = {
            "schema_version": "1.0",
        ...
        if cached:
            if json.loads(audit_path.read_text(encoding="utf-8")) != expected_audit:
                raise StageFailure("score", "Cached score audit changed")
        else:
            _atomic_json(audit_path, expected_audit)
        request_sha256 = _sha256(request_path)
        if self.config.strategy == "grit_score" and not cached:
            score_config = build_grit_spec(
                base_spec=self.config.value["training"]["base_spec"],
                input_parquet=values["target_manifest"],
                checkpoint=values["checkpoint"],
                output_dir=output_dir,
                settings=self.config.value["actions"]["score"].get("settings", {}),
            )
            values["score_config"] = score_config
        elif self.config.strategy == "grit_score":
            values["score_config"] = output_dir / "grit_score.yaml"
        ...
        def validate_scores() -> None:
            ...
            expected_commit = {
                ...
                "entrypoint_sha256": audit["entrypoint_sha256"],
        ...
        self._run_stage(
            ...
            stage_environment={
                ...
                "TAO_REFINEMENT_CLOSURE_VERSION": "1.0",
                "TAO_REFINEMENT_ENTRYPOINT_SHA256": entrypoint_sha256,
                "TAO_REFINEMENT_IMPLEMENTATION_SHA256": implementation_sha256,
            },
    ...
    def _train(
        ...
        checkpoint_template = "{output_dir}/checkpoint.pth"
        audit_path = output_dir / "train_implementation_audit.json"
        entrypoint_sha256 = self._locked_action_entrypoint("train")
        implementation_sha256 = self._locked_action_implementation("train")
        ...
        batch_size_per_gpu = resolved_training_batch_size(base_spec_path)
        ...
        training_spec, contract_path, _ = build_training_spec(
            ...
            output_dir=output_dir,
            num_nodes=int(allocation["nodes"]),
        ...
        def validate_checkpoint() -> None:
            self._validate_training_outputs(
                ...
                commit_path,
                output_dir / "_SUCCESS",
                expected_entrypoint=entrypoint_sha256,
                expected_implementation=implementation_sha256,
            )

        def native_training_finished() -> bool:
            ...
            try:
                ...
                source = output_dir / (
                    f"teacher_epoch_"
                    f"{int(current_contract['requested_data_passes']) - 1:03d}"
                    f"_step_{int(current_contract['total_optimizer_steps']):05d}.pth"
                )
                runtime_spec = Path(current_contract["runtime_spec"])
            except (KeyError, TypeError, ValueError, json.JSONDecodeError):
                return False
            return (
                source.is_file() and runtime_spec.is_file() and audit_path.is_file()
            )
        ...
        self._run_stage(
            ...
            finalize_outputs=lambda: finalize_training(output_dir),
            recover_outputs=native_training_finished,
    ...
    def _validate_training_outputs(
        ...
        *,
        ...
        audit_path = Path(str(contract.get("implementation_audit", "")))
        audit_expected = str(contract.get("implementation_audit_sha256", ""))
        if (
            not audit_path.is_file() or
            audit_path.stat().st_size != int(
                contract.get("implementation_audit_bytes", -1)
            ) or
            _sha256(audit_path) != audit_expected
        ):
            raise ValueError("Training implementation audit does not match its contract")
        audit = json.loads(audit_path.read_text(encoding="utf-8"))
        if (
            expected_entrypoint and
            audit.get("entrypoint_sha256") != expected_entrypoint
        ):
            raise ValueError("Training entrypoint differs from the controller lock")
        if (
            expected_implementation and
            audit.get("implementation_sha256") != expected_implementation
        ):
            raise ValueError("Training implementation differs from the controller lock")
        commit = json.loads(commit_path.read_text(encoding="utf-8"))
        commit_expectations = {
            "training_contract_sha256": _sha256(contract_path),
            "checkpoint_sha256": expected,
            "runtime_spec_sha256": runtime_expected,
            "implementation_audit_sha256": audit_expected,
        }
        if any(
    ...
    def _validate_completed_training(self, state: dict[str, Any]) -> None:
        for key, record in state.get("completed_stages", {}).items():
            ...
            required = {
                "checkpoint", "contract", "commit", "implementation_audit",
                "success",
            }
    ...
    def _validate_adopted_training(self) -> dict[str, Any]:
        ...
        previous_lock = json.loads(
            previous_data_lock.read_text(encoding="utf-8")
        )
        previous_adapters = previous_lock.get("adapter_files", {})
        entrypoint_records = [
            value
            for key, value in previous_adapters.items()
            if key.startswith("train.") and ".module." in key
        ]
        implementation_prefix = "train.implementation."
        implementation_records = {
            key.removeprefix(implementation_prefix): value["sha256"]
            for key, value in previous_adapters.items()
            if key.startswith(implementation_prefix)
        }
        if not entrypoint_records or not implementation_records:
            raise ValueError(
                "Continuation data lock is missing the native training "
                "entrypoint or implementation closure"
            )
        self._validate_training_outputs(
            checkpoint,
            contract_path,
            commit_path,
            marker,
            expected_entrypoint=str(entrypoint_records[0]["sha256"]),
            expected_implementation=(
                next(iter(implementation_records.values()))
                if len(implementation_records) == 1
                else canonical_digest(implementation_records)
            ),
        )
        ...
        return {
            ...
            "implementation_audit": {
                **_file_identity(Path(contract["implementation_audit"])),
                "path": str(Path(contract["implementation_audit"]).resolve()),
            },
        ...
        with self.store.controller_lock():
            ...
            self.store.complete_stage(
                ...
                outputs={
                    ...
                    "implementation_audit": adoption["implementation_audit"]["path"],
    ...
    def status(self) -> dict[str, Any]:
        ...
        state["controller_active"] = self.store.controller_active()
        state["job_statuses"] = {}
        for key, job in state.get("active_jobs", {}).items():
            try:
                result = self.runner.status(job["client_job_id"])
            except (AttributeError, RuntimeError, OSError, ValueError) as error:
                result = {"state": "UNKNOWN", "client_job_id": job["client_job_id"],
                          "error": str(error)}
            state["job_statuses"][key] = result
        state["resume_needed"] = (not state["controller_active"] and
    ...
    def _reconcile_cancellation(
        ...
        for key, job in list(state.get("active_jobs", {}).items()):
            ...
            expected_id = str(job["client_job_id"])
            if result.get("client_job_id") != expected_id:
                result = {
                    "state": "UNKNOWN",
                    "client_job_id": expected_id,
                    "error": "Runner cancellation acknowledgement identity mismatch",
                }
            results[key] = result
    ...
    def cancel(self) -> dict[str, Any]:
        """Persist intent and reconcile it with run-owned backend jobs."""
        intent = self.run_dir / "cancel.requested"
        intent.parent.mkdir(parents=True, exist_ok=True)
        intent.touch(exist_ok=True)
        state = self.store.load()
        if not state:
            return {"status": "canceling", "jobs": {}}
        ...
        try:
            ...
            for key, job in self.store.load().get("active_jobs", {}).items():
                expected_id = str(job["client_job_id"])
                try:
                    result = self.runner.cancel(expected_id)
                    if result.get("client_job_id") != expected_id:
                        raise RuntimeError(
                            "Runner cancellation acknowledgement identity mismatch"
                        ) from None
                    results[key] = result
                except Exception as exc:
                    results[key] = {
