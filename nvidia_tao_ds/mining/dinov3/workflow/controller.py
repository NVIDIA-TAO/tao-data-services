    ...
    def _data_lock(self) -> dict[str, Any]:
        ...
        for action_name in ("score", "data", "search", "train", "evaluate"):
            ...
            if native_subtask in {"grit_score", "train"}:
                ...
                required_files = [package_root / "config/dinov3/default_config.py"]
                ...
                for implementation in sorted(
                    ...
                    if not implementation.is_file():
                        raise ValueError(
                            f"Native DINOv3 implementation closure is incomplete: {implementation}"
                        )
                    relative = implementation.relative_to(package_root).as_posix()
                    adapter_files[
                        f"{action_name}.implementation.native.{relative}"
                    ] = {
    ...
    def _train(
        ...
        def native_training_finished() -> bool:
            ...
            try:
                ...
                return False
            return source.is_file() and runtime_spec.is_file()
