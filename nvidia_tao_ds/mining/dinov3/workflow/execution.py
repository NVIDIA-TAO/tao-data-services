...
from nvidia_tao_ds.mining.dinov3.contracts import write_json_atomic
...
class Runner(Protocol):
    ...
    def status(self, client_job_id: str) -> dict[str, Any]:
        """Read the live backend state without submitting."""
...
def client_job_id(
    *, run_scope: str, run_id: str, round_index: int, stage: str,
    command: list[str],
) -> str:
    """Derive a stable job identity scoped to one durable workflow directory."""
    payload = json.dumps(
        [run_scope, run_id, round_index, stage, command],
        separators=(",", ":"),
        ensure_ascii=True,
    ).encode("utf-8")
...
class LocalRunner:
    ...
    def __init__(self, jobs_dir: str | Path, max_attempts: int = 3):
    ...
    def preflight(self, request: StageRequest) -> None:
        """Validate allocation and capabilities without reading data or launching."""
        ...
        if requested_gpus:
            if allocated_visible is None:
                raise RuntimeError("Local GPU execution requires a platform-pinned CUDA_VISIBLE_DEVICES allocation")
            visible_count = len([
                device for device in str(allocated_visible).split(",") if device.strip()
            ])
    ...
    def run(self, request: StageRequest) -> StageResult:
        ...
        requested_gpus = int(request.resources.get("gpus_per_node", request.resources.get("gpus", 0)))
        allocated_visible = os.environ.get("CUDA_VISIBLE_DEVICES")
        ...
        if record_path.is_file():
            ...
            if record.get("state") == "RUNNING":
                ...
                if pid is not None and self._group_alive(pid):
                    raise RuntimeError("Orphaned local job still has live descendants; manual cleanup is required before retry")
        if cancel_path.exists():
        ...
        node_environment = request.execution_contract.get("node_environment", {})
        if node_environment.get("contract") == "lightning_node_entrypoint_v1":
            # The local backend owns this allocation. Never inherit scheduler or
            # user rendezvous values that could wait for nonexistent peers.
            environment["WORLD_SIZE"] = "1"
            environment["NUM_GPU_PER_NODE"] = str(requested_gpus)
            environment["NODE_RANK"] = "0"
            environment["MASTER_ADDR"] = "127.0.0.1"
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
                listener.bind((environment["MASTER_ADDR"], 0))
                environment["MASTER_PORT"] = str(listener.getsockname()[1])
        ...
        if attempt > self.max_attempts:
            raise RuntimeError(f"Local job {request.client_job_id} exhausted max_attempts={self.max_attempts}; inspect its log before starting a new run")
        write_json_atomic(self.jobs_dir / f"{request.client_job_id}.attempt-{attempt}.json", {
            "client_job_id": request.client_job_id, "attempt": attempt,
            "CUDA_VISIBLE_DEVICES": allocated_visible,
            "retry": attempt > 1,
        })
    ...
    def status(self, client_job_id: str) -> dict[str, Any]:
        """Report durable and live process state without relaunching a job."""
        path = self.jobs_dir / f"{client_job_id}.json"
        if not path.is_file():
            return {"state": "UNKNOWN", "client_job_id": client_job_id}
        record = json.loads(path.read_text(encoding="utf-8"))
        if record.get("state") in {"RUNNING", "CANCELING"}:
            pid, start = self._process_reference(str(record.get("backend_ref", "")))
            alive = pid is not None and start is not None and self._alive(pid, start)
            record["leader_alive"] = alive
            if not alive:
                record["state"] = "UNKNOWN"
                record["native_state"] = "ORPHANED"
                record["descendants_alive"] = pid is not None and self._group_alive(pid)
        return record
    ...
    def cancel(self, client_job_id: str) -> dict[str, Any]:
        ...
        if not record_path.is_file():
            canceled = StageResult(
                ...
                backend_ref="local:not-started",
                return_code=None,
                log_path=str(self.jobs_dir / f"{client_job_id}.log"),
                native_state="CANCELED_BEFORE_START",
            )
            self._write_record(record_path, canceled)
            return asdict(canceled)
        record = json.loads(record_path.read_text(encoding="utf-8"))
        if record.get("state") in {"RUNNING", "CANCELING"}:
            pid, start_time = self._process_reference(
                str(record.get("backend_ref", ""))
...
class ExternalRunner:
    ...
    def status(self, client_job_id: str) -> dict[str, Any]:
        """Read and identity-check a platform job without submitting."""
        result = self._call("status", "--client-job-id", client_job_id)
        if result.get("client_job_id") != client_job_id:
            raise RuntimeError("Runner status identity mismatch")
        return result
    ...
    def run(self, request: StageRequest) -> StageResult:
        ...
        while True:
            ...
            if status.get("client_job_id") != request.client_job_id:
                raise RuntimeError(
                    "Runner status acknowledgement identity mismatch for "
                    f"{request.client_job_id}"
                )
            ...
            if state not in allowed_states:
                raise RuntimeError(
                    f"Runner returned invalid state {state!r} for "
                    f"{request.client_job_id}"
                )
    ...
    def cancel(self, client_job_id: str) -> dict[str, Any]:
        """Reconcile cancellation intent with the run-owned process or backend job."""
        result = self._call("cancel", "--client-job-id", client_job_id)
        if result.get("client_job_id") != client_job_id:
            raise RuntimeError(
                "Runner cancellation acknowledgement identity mismatch for "
                f"{client_job_id}"
            )
        ...
        if state not in {
            ...
            raise RuntimeError(
                f"Runner returned invalid cancel state {state!r} for "
                f"{client_job_id}"
            )
        return result
...
def build_runner(config: dict[str, Any], run_dir: Path) -> Runner:
    ...
    if config["backend"] == "local":
        return LocalRunner(run_dir / "jobs", max_attempts=int(config.get("max_attempts", 3)))
