...
def build_training_spec(
    *,
    ...
    if not spec.dataset.train_dataset.images_dir:
        spec.dataset.train_dataset.images_dir = "/"
    ...
    batch_size = int(spec.dataset.batch_size)
...
def finalize_training(output_dir: str | Path) -> Path:
    ...
    source = output / f"teacher_epoch_{expected_epoch:03d}_step_{expected_step:05d}.pth"
    if not source.is_file() or not re.fullmatch(
        r"teacher_epoch_\d{3}_step_\d{5,}\.pth", source.name
    ):
        raise RuntimeError(
            "Native DINOv3 train did not publish the expected terminal EMA teacher: "
            f"{source}"
        )
