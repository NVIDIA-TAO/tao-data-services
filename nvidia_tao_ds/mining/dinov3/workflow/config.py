...
class WorkflowConfig:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> "WorkflowConfig":
        ...
        data = value["data"]
        data.setdefault("source_store_validation", "sealed_inventory")
        ...
        execution.setdefault("max_attempts", 3)
        if (isinstance(execution["max_attempts"], bool) or
                not isinstance(execution["max_attempts"], int) or execution["max_attempts"] <= 0):
            raise ValueError("execution.max_attempts must be a positive integer")
        ...
        for action_name, action in (
            ...
            reserved = {"checkpoint", "output_dir", "target_manifest", "score_config", "training_spec",
                        "training_manifest", "benchmark_manifest", "evaluation_scope", "round"}
            collisions = reserved.intersection(action.get("parameters", {}))
            if collisions:
                raise ValueError(f"actions.{action_name}.parameters cannot override reserved placeholders: {sorted(collisions)}")
            if "backend" in action and action_name == "train":
                raise ValueError("Use actions.train.execution_mode; actions.train.backend is unsupported")
            if execution["backend"] == "local":
                for local_action in (action, action.get("candidate", {}), action.get("rerank", {})):
                    if local_action.get("command", [])[:1] in (["python"], ["python3"]):
                        local_action["command"][0] = sys.executable
