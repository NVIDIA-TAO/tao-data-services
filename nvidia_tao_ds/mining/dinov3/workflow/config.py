...
class WorkflowConfig:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> "WorkflowConfig":
        ...
        if (
            workflow["initialization"] == "parent_checkpoint" and
            not data.get("previous_training_manifest")
        ):
            raise ValueError(
                "parent_checkpoint initialization requires "
        ...
        training.setdefault("lr_scaling_rule", "sqrt_global_batch")
        if training["lr_scaling_rule"] != "sqrt_global_batch":
            raise ValueError(
                "training.lr_scaling_rule must be sqrt_global_batch"
            )
        explicit_checkpoint_policy = "checkpoint_policy" in training
        legacy_warm_start = training.get("warm_start")
        if legacy_warm_start is not None and not isinstance(
            legacy_warm_start, bool
        ):
            raise ValueError("training.warm_start must be a boolean")
        if not explicit_checkpoint_policy:
            training["checkpoint_policy"] = (
                "previous_round_checkpoint"
                if legacy_warm_start is True
                else "base_checkpoint_each_round"
            )
        if training["checkpoint_policy"] not in {
            "base_checkpoint_each_round",
            "previous_round_checkpoint",
        }:
            raise ValueError(
                "training.checkpoint_policy must be base_checkpoint_each_round "
                "or previous_round_checkpoint"
            )
        uses_previous_checkpoint = (
            training["checkpoint_policy"] == "previous_round_checkpoint"
        )
        if (
            explicit_checkpoint_policy and
            legacy_warm_start is not None and
            bool(legacy_warm_start) != uses_previous_checkpoint
        ):
            raise ValueError(
                "training.warm_start conflicts with training.checkpoint_policy"
            )
        # The boolean is accepted as a legacy input but normalized away so the
        # resolved run has one unambiguous checkpoint contract.
        training.pop("warm_start", None)
        ...
        score_module = "nvidia_tao_pytorch.ssl.dinov3.data_refinement.cli"
        score_command = list(map(str, score["command"]))
        uses_builtin_grit = any(
            token == "-m" and
            index + 1 < len(score_command) and
            score_command[index + 1] == score_module
            for index, token in enumerate(score_command)
        )
    ...
    def plan(self) -> dict[str, Any]:
        ...
        training_summary = (
            "initialize from the immutable base checkpoint -> train on the "
            "cumulative manifest"
            if checkpoint_policy == "base_checkpoint_each_round"
            else "warm-start from the previous round checkpoint -> train on the "
            "cumulative manifest"
        )
        action_execution = {
            name: {
        ...
        return {
            ...
            "initialization": (
                ...
                else "warm_start_from_parent_checkpoint"
            ),
            "round_stages": stages,
