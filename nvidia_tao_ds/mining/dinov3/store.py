...
def bind_store_payload_contract(
    *,
    ...
    if canonical_digest(shards) != source.get("inventory_digest"):
    ...
    accepted_prefixes = tuple(f"{root.rstrip('/')}/" for root in root_strings)
    for shard in shards:
        ...
        for batch in parquet.iter_batches(batch_size=65536, columns=selected):
            ...
            canonical = locator_text.str.startswith("/") & ~locator_text.str.contains(
                r"/(?:\.{1,2})(?:/|$)|//", regex=True
            )
            belongs = locator_text.isin(root_strings) | locator_text.str.startswith(
                accepted_prefixes
            )
            valid = canonical & belongs
            if not valid.all():
                invalid = locator_text[~valid].iloc[0]
                raise ValueError(
                    "Source locator is non-canonical or outside every contracted root: "
                    f"{invalid}"
                )
    ...
    payload["content_verification"] = {
        ...
        "shard_seal_digest": canonical_digest(content_seals),
    ...
    write_json_atomic(destination / "embedding_store.json", payload)
                ...
                for batch in parquet.iter_batches(
                    ...
                    if payload_contract is not None:
                        ...
                        for storage_type, locator, member in zip(
                            ...
                            locator_count += 1
                    vectors = np.asarray(
                        frame[embedding_column].tolist(), dtype=np.float32
                    )
                    if vectors.ndim != 2 or not np.isfinite(vectors).all():
                        raise ValueError(f"{path} embeddings must be a finite matrix")
                    norms = np.linalg.norm(vectors, axis=1)
                    if not np.isfinite(norms).all():
                        raise ValueError(f"{path} embedding norms contain non-finite values")
                    if np.any(norms == 0):
                        raise ValueError(f"{path} contains a zero embedding")
