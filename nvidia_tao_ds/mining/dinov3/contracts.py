...
def vector_matrix(values: Any, *, label: str = "Embedding") -> np.ndarray:
    ...
    try:
        matrix64 = np.asarray(
            values.tolist() if hasattr(values, "tolist") and not isinstance(values, np.ndarray) else values,
            dtype=np.float64,
        )
...
def artifact_publication(output_dir: str | Path):
    ...
    if key in held:
        ...
        try:
            ...
            held[key][1] -= 1
        return
    # Keep the lock outside the sealed payload. Never unlink a lock inode:
    # waiters may still hold it while a subsequent publisher arrives.
    lock = root.with_name(f".{root.name}.artifact.lock").open("a+b")
